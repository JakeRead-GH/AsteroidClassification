...
\noindent The dataset contains orbital and physical properties of thousands of discovered and analyzed asteroids. There is a target label \texttt{pha} present, representing the classification to be predicted by the model. The features that will be used in the model are enumerated in in \hyperref[sec:Features]{Features}. The dataset also contains identifier, metadata, epoch and reference data features that will not be used in our model. The excluded features are: 
