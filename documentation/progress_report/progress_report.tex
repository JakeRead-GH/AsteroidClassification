...
\medskip \noindent As the project progresses, the group has several plans of improvements to make. 
\begin{itemize}
    \item We will work on refining our model by running with different feature sets to see which have the greatest impact on its performance. We have the ability to pass different sets through the command line, but have not extensively tested the results. 
    \item We will experiment with more rigorous hyperparameter tuning, perhaps a few thousand trials over a few hours rather than the 10 we are using now. 
    \item We will consider a focus on recall instead of F1 score. By focusing on recall we are minimizing the number of hazardous asteroids that the model misclassifies by penalizing more harshly for missed positives than false negatives. 
    \item We will work with different techniques to handle the class imbalance due to the much smaller number of hazardous asteroids in our dataset
\end{itemize}
